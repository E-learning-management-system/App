...
class RecordHomeWorkController extends ChangeNotifier {
  ...
  Future newExercise(id) async {
    ...
    print("response status code(new exercise)=   " +
  ...
  Future deleteExercise(id) async {
    ...
    print("jsonDecode(delete exercise)=   " +
  ...
  Future newAnswer(id) async {
    ...
    var response = await http.post(Uri.parse(url),
        ...
        body: jsonEncode({"description": des, "file": file}));

    print("jsonDecode(new answer)=   " +
        const Utf8Decoder().convert(response.bodyBytes));
    if (response.statusCode == 201) {
      notifyListeners();
      return true;
  ...
  Future downloadFile(file, id) async {
    ...
    if (statuses[Permission.storage]!.isGranted) {
      ...
      var response = await Dio().download(file, savePath,
  ...
  void showDate(BuildContext context) async {
    date = await jalaliCalendarPicker(
        context: context,
        showTimePicker: true,
